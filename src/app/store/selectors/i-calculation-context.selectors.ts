...
export const groupsState = (state: any) =>
  state[fromIGroups.groupFeatureKey] as fromIGroups.State;
export const ordersState = (state: any) =>
  state[fromIOrders.orderFeatureKey] as fromIOrders.State;
export const productsState = (state: any) =>
  state[fromIProducts.productFeatureKey] as fromIProducts.State;
...
export const selectCalculationContextValid = createSelector(
  ...
  (calculationAttributesState, groupsState, ordersState, productsState) => {
    return (
      ...
      groupsState.ids.length > 0 &&
      ordersState.ids.length > 0 &&
      productsState.ids.length > 0
    );
  }
);
