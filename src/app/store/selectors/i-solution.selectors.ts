...
export const solutionsState = (state: any) =>
  state[solutionFeatureKey] as State;
...
export const selectNextSolutionId = createSelector(
  ...
  (state: State) => {
    if (typeof state.selectedSolutionId === 'string') {
      const index = Object.values(state.entities).findIndex(
        entry => entry!.id === state.selectedSolutionId
      );
...
export const selectSolutionById = (solutionGuid: string | null) =>
  createSelector(solutionsState, (state: State) => {
    const solution = Object.values(state.entities ?? {}).find(
      solution => solution?.id === solutionGuid
    );
    return solution ?? null;
  });

export const selectSolutionByAlgorithm = (
  ...
  createSelector(solutionsState, (state: State) => {
    ...
    const solution = Object.values(state.entities ?? {}).find(
      solution => solution?.calculationSource?.title === algorithm
    );
    return solution ?? null;
  });

export const selectSolutions = createSelector(solutionsState, (state: State) =>
  (Object.values(state.entities) as ISolution[]).sort((solutionA, solutionB) =>
    solutionA.calculated > solutionB.calculated ? 1 : -1
  )
);
...
export const selectCurrentSolutionValidation = createSelector(
  ...
  (state: State) => {
    const currentSolution =
      state.entities[state.selectedSolutionId as any] ?? null;
    if (!currentSolution) {
      return null;
    }
