...
export class ContainerPreviewComponent implements OnChanges, OnInit {
      ...
      tooltip: {
        callbacks: {
          label: tooltipItem => {
            ...
            const total: number = tooltipItem.dataset.data.reduce(
  ...
  private _calculateUsedSpace() {
    ...
    const used = this.container.goods.reduce(
      (x, curr) => (x += curr.length * curr.width * curr.height),
      0
    );
    const total =
      this.container.height * this.container.length * this.container.width;
    this.datasets = [
