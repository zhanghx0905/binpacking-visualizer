...
export class SceneVisualizationComponentService {
  ...
  public getPointedElement(
    event: MouseEvent,
    scene: ThreeJS.Scene
  ): null | ThreeJS.Intersection {
    const meshes = scene.children.filter(
      child => child instanceof ThreeJS.Mesh
    ) as ThreeJS.Mesh[];
    const x =
      (event.offsetX / (event.target as HTMLCanvasElement).offsetWidth) * 2 - 1;
    ...
    intersects.forEach(object => {
      if (
        object.object instanceof ThreeJS.Mesh &&
        (found === null || found.distance > object.distance)
      ) {
        found = object;
      }
    });
  ...
  public setUpCamera(height: number, width: number) {
    if (!this._camera) {
      this._camera = new ThreeJS.PerspectiveCamera(
        20,
        width / height,
        1,
        10000000
